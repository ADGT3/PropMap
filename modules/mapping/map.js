    ...
    rows.forEach(p => {
      if (p.domain_listing_id) {
        ...
        const entry = {
          ...
          state:   p.state || 'NSW',
          lot_dps: p.lot_dps,
          domain_listing_id: String(p.domain_listing_id),
        ...
        _linkedPropertyList.push(entry);
      }
...
function _listingIdSetFromList(arr) {
  const seen = new Set();
  (arr || []).forEach(l => {
    _domainListingKeys(l && l.id).forEach(k => seen.add(k));
    _domainListingKeys(l && (l.listingUrl || l.listing_url)).forEach(k => seen.add(k));
  });
  return seen;
}

function _allLinkedPropertyEntries() {
  const byProp = new Map();
  _propertyByDomainId.forEach((e, mapKey) => {
    if (!e) return;
  ...
  _linkedPropertyList.forEach(e => {
    if (!e) return;
    const uniq = e.id || e.domain_listing_id;
    if (uniq && !byProp.has(uniq)) byProp.set(uniq, e);
  });
  return [...byProp.values()];
}

function _linkedEntriesInView(bounds) {
  if (!bounds || typeof bounds.contains !== 'function') return [];
  return _allLinkedPropertyEntries().filter(e => {
    if (e.lat == null || e.lng == null) return false;
    return bounds.contains(L.latLng(e.lat, e.lng));
  });
}
...
function clearParcelSelection() {
  _activePipelineDealId = null;
  if (typeof window.refreshPipelinePins === 'function') {
    try { window.refreshPipelinePins(); } catch (_) {}
  }
...
async function selectPropertyAtPoint(latlng, includeSrlup, includeZoning, includeFlood, includeRoads, listing = null, addToSelection = false) {
  ...
  // Multi-select uses blue numbered pins; single active property uses green fill + white outline
  // (same green as Domain outline / selected pipeline pins).
  const pinColor = addToSelection ? '#1a4a8a' : MARKER_COLOR_ACTIVE;
  const pinHtml  = addToSelection
    ? `<div class="search-pin" style="background:${pinColor};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700;width:28px;height:28px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:2.5px solid rgba(255,255,255,0.8)"><span style="transform:rotate(45deg)">${pinNum}</span></div>`
    : `<div class="search-pin" style="background:${pinColor};width:28px;height:28px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:2.5px solid rgba(255,255,255,0.8);box-shadow:0 2px 8px rgba(0,0,0,0.5)"></div>`;
...
// Pin palette
//   Gold  (#c4841a) — unselected Domain / pipeline markers
//   Green (#1a6b3a) — active/selected property pin fill AND Domain outline
//                     (same green as the manual search / click pin)
const MARKER_COLOR_GOLD   = '#c4841a';
const MARKER_COLOR_ACTIVE = '#1a6b3a';
const MARKER_COLOR        = MARKER_COLOR_GOLD; // legacy alias
const PIN_OUTLINE_DOMAIN  = MARKER_COLOR_ACTIVE; // Domain ring = active green
const PIN_OUTLINE_DEFAULT = 'rgba(255,255,255,0.8)';

// Currently selected pipeline deal id (star pin); cleared with parcel selection.
let _activePipelineDealId = null;

/**
 * Teardrop map pin.
 * @param {string} [color]  fill override (defaults: gold unselected, green selected)
 * @param {object} [opts]
 * @param {boolean} [opts.domain=false]    Domain-linked → green outline
 * @param {boolean} [opts.star=false]      pipeline → ★
 * @param {boolean} [opts.selected=false]  active selection → green fill
 * @param {string}  [opts.outline]         outline override
 *
 * Legend (fill / star / outline):
 *   Domain listing, not selected     → gold / — / green
 *   Pipeline only, not selected      → gold / ★ / white
 *   Pipeline + Domain, not selected  → gold / ★ / green
 *   Selected (manual/search, no Dom) → green / — / white
 *   Pipeline selected, no Domain     → green / ★ / white
 *   Pipeline selected + Domain       → green / ★ / green
 *   Domain listing selected          → green / — / green
 */
function makeIcon(color, opts = {}) {
  // Allow makeIcon(opts) with no color arg
  if (color && typeof color === 'object' && !opts.domain && !opts.star && opts.selected == null && !opts.outline) {
    opts = color;
    color = null;
  }
  ...
  const fill     = color || (selected ? MARKER_COLOR_ACTIVE : MARKER_COLOR_GOLD);
  const outline  = opts.outline || (domain ? PIN_OUTLINE_DOMAIN : PIN_OUTLINE_DEFAULT);
  const inner    = star
    ? `<span style="transform:rotate(45deg);line-height:1;color:#fff;font-size:12px">★</span>`
    : '';
  const cls = ['map-pin'];
  if (domain) cls.push('map-pin-domain');
  if (selected) cls.push('map-pin-selected');
  if (star) cls.push('map-pin-pipeline');
  return L.divIcon({
    className: cls.join(' '),
    html: `<div style="
...
/** Refresh Domain listing marker icons to reflect _activeListingId selection. */
function _syncListingMarkerIcons() {
  if (!markers || typeof markers !== 'object') return;
  Object.keys(markers).forEach(id => {
    const m = markers[id];
    if (!m || !m.setIcon) return;
    const l = (listings || []).find(x => String(x.id) === String(id));
    const isDomain = !l || l._source !== 'corelogic';
    const selected = _activeListingId != null && String(_activeListingId) === String(id);
    m.setIcon(makeIcon(null, { domain: isDomain, selected }));
  });
}
...
window._renderPipelinePins = function () {
  ...
  Object.entries(pipelineData).forEach(([id, item]) => {
    ...
    // Pipeline star pin. Green outline if Domain-linked; green fill if selected.
    const linkedToDomain = !!(
      p.domain_id || p.domain_listing_id || p.listing_url || p._listingUrl
    );
    const isSelected = _activePipelineDealId != null && String(_activePipelineDealId) === String(id);

    const icon = makeIcon(null, {
      star: true,
    ...
    marker.on('click', async () => {
      _activePipelineDealId = id;
      _activeListingId = null;
      _syncListingMarkerIcons();
      // Rebuild star pins so only this deal is green-selected; others return to gold
      if (typeof window.refreshPipelinePins === 'function') {
        try { window.refreshPipelinePins(); } catch (_) {}
      }
