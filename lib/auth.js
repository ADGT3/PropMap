 ...
 * admin/module access checks (modules: mapping, pipeline, crm, finance).
 *
...
/**
 * Guard: session must include the given app module (or admin / legacy full access).
 * Module ids: mapping | pipeline | crm | finance  (see lib/modules.js)
 */
export function requireModule(session, res, moduleId) {
  if (!session) { sendUnauthorized(res); return false; }
  if (isAdmin(session)) return true;
  const mods = session.modules || [];
  if (mods.includes('*') || mods.includes('propmap')) return true; // legacy full access
  if (mods.includes(moduleId)) return true;
  sendForbidden(res, `Module access required: ${moduleId}`);
  return false;
}

/** Require any one of the listed modules. */
export function requireAnyModule(session, res, moduleIds) {
  if (!session) { sendUnauthorized(res); return false; }
  if (isAdmin(session)) return true;
  const mods = session.modules || [];
  if (mods.includes('*') || mods.includes('propmap')) return true;
  if (moduleIds.some(m => mods.includes(m))) return true;
  sendForbidden(res, `Module access required: one of ${moduleIds.join(', ')}`);
  return false;
}
