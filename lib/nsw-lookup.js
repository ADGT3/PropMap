...
export async function lookupByLotDP(lotDpString) {
  ...
  const lotParams = new URLSearchParams({
    ...
    outFields:      'lotidstring,lotnumber,sectionnumber,planlabel,planlotarea,Shape__Area',
    returnGeometry: 'true',
    outSR:          '4326',
  });
  ...
  try {
    ...
    console.warn(`[nsw-lookup] lotDP query failed for ${normalized}:`, err?.message || err);
    return null;
  }
