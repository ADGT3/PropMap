...
| Version | Notes |
|---|---|
| V75.4 | **Parcels as first-class entity.** Introduces Parcel — a container for 2+ adjacent/related Properties, used for multi-property acquisition deals. A Parcel has contacts, notes, and a single Deal at a time; removing a Property detaches it without deleting it. **Schema**: new `parcels` table (`id, name, not_suitable_until, not_suitable_reason`), new nullable FK `properties.parcel_id`, new nullable FK `deals.parcel_id` with CHECK constraint `(property_id IS NULL) <> (parcel_id IS NULL)` — every deal targets exactly one of a Property or a Parcel. `entity_contacts.entity_type` now accepts `'parcel'`. `notes.entity_type` now accepts `'parcel'`. **Migration** (`api/migrate-to-v75-4.js`): splits the 4 existing synthetic multi-parcel Property rows (660-670 Bringelly Rd, 16-48 Loftus Rd, 1146-1178 The Northern Rd, 109-115 Deepfields Rd) into a real Parcel + N Properties each. Re-points the matching acquisition Deals from `property_id` → `parcel_id`. Re-points entity_contacts and notes from synthetic property to new parcel. Deletes the synthetic property rows. Idempotent via `_migrations` table. **New `api/parcels.js`** endpoint with GET list/one/expanded, POST create/set_not_suitable/clear_not_suitable/add_property/remove_property, PUT rename, DELETE (refused if deals exist). **`api/deals.js`** extended: accepts `parcel_id` in queries and creates; new action `new_on_parcel`; GET responses include joined `parcel` object + `parcel_properties[]` array when applicable. **`api/contacts.js` + `api/notes.js`** enrichment queries handle deals-on-parcels and `entity_type='parcel'`. **Frontend — new `lib/parcel-format.js`**: `formatParcelTitle(properties)` utility collapses contiguous street numbers into ranges, groups by street, handles unit numbers (2/14), Lot fallback, multi-street with ' & ' joiner. Unit-tested. **CRM tabs** now: Contacts · Properties · **Parcels** · Organisations. + Add button is per-tab (Contacts/Organisations show Create modal; Properties/Parcels hide the button since they're created via map ⌘-click or the pipeline flow). Parcels tab list shows Title (merged address), Property count, Active deal stage badge, Not Suitable badge. Click row opens **Parcel modal** with collapsible sections: Parcel Details (editable name; merged title display; total area; ID), Not Suitable (with 30d/90d/6m/1y/Permanent snooze controls), Properties-in-parcel (numbered list with Remove button per row), Deals (list of all deals on this parcel; smart button shows "Open Active Deal" or "+ New Deal (history: N closed)" based on state), Contacts (linked via `entity_contacts` with `entity_type='parcel'`), Notes (parcel-scoped, entity_type='parcel'). **Properties tab** is a V75.5 placeholder with "Coming in V75.5" note. **window.CRM.navigateTo(subRoute, entityId)** added — enables router deep links to `/crm/parcels/<id>`. **Kanban**: `dealRowToInternal` fully handles parcel deals by aggregating `parcel_properties[]` into a pseudo-property (merged title via formatter, total area, averaged centroid, union of lot polygons for map rendering). `internalToDealPayload` sets `parcel_id` instead of `property_id` when `entry._isParcel`. `dbSave` skips property upsert for parcel deals. `dbDelete` deletes the deal directly for parcel deals, preserving the Parcel entity. **Map pin numbering fix**: the `_selectedParcels` promotion on the first ⌘-click now correctly captures the provisional first selection even when async reverse-geocode has nulled `clickMarker` — reconstructs the pin from persisted `clickMarkerData`, paints blue-#1, then the current click drops blue-#2. **Not yet implemented in V75.4** (deferred to V75.4a or V75.5): map ⌘-click → "+ Pipeline" does NOT yet create a real Parcel + N Properties. The legacy synthetic-property-with-JSONB-parcels flow is preserved for now. New multi-property acquisitions will continue to create synthetic properties that would need a future follow-up migration to become real Parcels. Existing V75.4 migration handles all 4 of the current prod synthetic properties correctly. **Deploy sequence**: push all files to preview → GET `/api/migrate-to-v75-4` (dry-run should show 4 candidates) → POST to execute → verify 4 parcels in CRM Parcels tab, verify Kanban cards show merged titles, verify contact links are intact → promote to prod → run migration on prod. Rollback = restore Neon branch snapshot. |
