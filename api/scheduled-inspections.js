    ...
    switch (req.method) {
      ...
      case 'GET': {
        ...
        if (date) {
          ...
          // check in for. Read-only convenience query.
          //
          // Date is resolved server-side in Australia/Sydney TZ to avoid
          // client-clock-skew off-by-one. Each branch builds a separate
          // self-contained query (the neon adapter doesn't compose embedded
          // `sql` fragments — V77.2 migration learned this the hard way).
          let rows;
          if (date === 'today') {
            rows = await sql`
              SELECT si.*,
              ...
              WHERE si.scheduled_date = (now() AT TIME ZONE 'Australia/Sydney')::date
            ...
            rows = await sql`
              ...
              LEFT JOIN properties p ON p.id  = d.property_id
              LEFT JOIN parcels    par ON par.id = d.parcel_id
              WHERE si.scheduled_date = ${date}::date
              ORDER BY si.scheduled_date ASC, si.start_time ASC, si.id ASC`;
          } else {
